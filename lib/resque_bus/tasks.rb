...
namespace :queuebus do
  ...
  task :preload do
    ...
    # Loads the environment and such if defined
  ...
  namespace :example do
    ...
    task publish: ['queuebus:preload', 'queuebus:setup'] do
      which = %w(one two three other)[rand(4)]
      QueueBus.publish("event_#{which}", 'rand' => rand(99_999))
      QueueBus.publish('event_all', 'rand' => rand(99_999))
      QueueBus.publish('none_subscribed', 'rand' => rand(99_999))
